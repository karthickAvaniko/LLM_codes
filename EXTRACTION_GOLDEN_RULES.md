...
> Last updated: 2026-08-31 | Source of truth: `gateway/main.py`, `EXTRACTION_RULES` constant (~line 675)
...
15. **Follow cross-reference pointers, never copy them as the value — and never let "can't find it" become the default** *(added 2026-08-26, extended same day; reopened and hardened 2026-08-31)* — some fields are printed as a POINTER to where the real value is, not the value itself (e.g. a `SHIP TO:` field reading "See Address on Lines Below", with the actual address appearing elsewhere on the page, often near the line items under a label like "Primary Location"). Find and extract the real value the pointer refers to; never store the pointer/instruction phrase itself ("See Address on Lines Below", "See Attached", "As Noted Above") as if it were the data — **and never silently drop the field to an empty object either; an empty `{}` is exactly as wrong as storing the pointer text when the real value is genuinely printed elsewhere on the page.** Before concluding a pointer's target is unfindable, you must actually look for it: scan near the line items, near any "Primary Location"/"Ship From"/"Location"-style label, and anywhere else on the page a location/address block could sit. "Genuinely can't be found" means the value is not printed anywhere on the page at all — not merely that it wasn't spotted on a first pass. Only after that active search comes up empty should the field be omitted rather than filled with the pointer text; omission is a last resort, not a shortcut. Added after a real invoice (`INV91679`) came back with `"ship_to": {"note": "See Address on Lines Below"}` — the real ship-to address (`333 Route 17 North, Mahwah NJ 07430`) was sitting right there in the document under "Primary Location," near the line items, and never got extracted. **Extended the same day** to also cover the cross-*page* version of this pattern: a totals/subtotal row showing only "Continued" (or "Cont'd"/"See Next Page") where the number should be, because the real figure is printed on a later continuation page — never store the literal word "Continued" as a total's value, go find the real number on the continuation page instead. Added after a real invoice (`102391`) came back with `"invoice_subtotal": "Continued"` instead of the real `518.00` that was printed on page 3. **Reopened 2026-08-31** — a second real invoice (`INV90893`), same "SHIP TO: See Address on Lines Below" pointer with the real address again printed under "Primary Location" near the line items, came back with `"ship_to": {}` — empty, instead of either the pointer text or the real address. This is a new failure *shape* of the same underlying gap: the model correctly stopped copying the pointer phrase (the original fix held) but never took the second step of actually resolving it (the actual point of the rule), so the omission escape hatch quietly became the default outcome instead of a genuine last resort. The rule text above was hardened accordingly — the original 2026-08-26 wording only forbade the wrong *value*, not the wrong *absence*.
...
| Ship-to address left as `{"note": "See Address on Lines Below"}` instead of following that pointer to the real address printed elsewhere on the page (real invoice `INV91679`) | ⚠️ **Reopened 2026-08-31** — fix held against the *pointer-text-copy* shape, but a second invoice (`INV90893`) hit a new shape of the same gap: `"ship_to": {}` (silently empty, real address never resolved). Rule #15 hardened same day to explicitly forbid the empty-object escape hatch as a default. Needs re-verification on both invoices before calling it closed again. |
